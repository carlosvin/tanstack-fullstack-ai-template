...
2. **Schemas as the type source:** Wire and tool shapes use a runtime validator (Zod, ArkType, Valibot, …) with **schema-inferred types**. Hand-written interfaces define **behavior** (`Repository`, `AIAdapterService`, …), not ad-hoc JSON types.
...
Auth for writes lives on **POST server functions** (`requireAuthMiddleware`), not on a second repository type. One `Repository` interface is enough; split read/write implementations only if a concrete backend needs it (for example an overlay decorator).
...
- **Overlay repository:** a `Repository` decorator over a read-only upstream plus sparse user overrides; pure `applyOverrides`; writes only to overrides. This is optional composition — not a Core Contract read/write factory split.
