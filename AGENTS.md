...
**CI/CD (GitHub Actions + Netlify CLI):** `.github/workflows/ci.yml` runs **lint, test, build**, then deploys via `netlify deploy` (bypasses Netlify Git clone). Requires GitHub secrets `NETLIFY_AUTH_TOKEN` and `NETLIFY_SITE_ID`. **Production** deploys on push to `main` after validation; **deploy previews** on pull requests (with PR comment). Build settings: `netlify.toml` → `pnpm build`, publish `.output/public`. If Netlify Git integration is healthy, disable automatic builds in the Netlify dashboard (Build & deploy → Continuous deployment → Stop builds) to avoid duplicate deploys.

**Phase 5 checklist:** `pnpm format && pnpm lint && pnpm test && pnpm build` → merge → GHA production deploy → smoke-test [demo](https://leafy-manatee-16b96c.netlify.app). **Netlify env** (post-#6): `SENTRY_DSN`, `ENV`, `LOG_LEVEL`, `REPOSITORY_TYPE=seed`, `OPENAI_API_KEY` (or chosen provider) — not `VITE_SENTRY_DSN`.
...
- **Deploys via GitHub Actions + Netlify CLI.** GHA validates then runs `netlify deploy` (production on `main`, previews on PRs). Requires `NETLIFY_AUTH_TOKEN` and `NETLIFY_SITE_ID` repo secrets. If Netlify Git auto-builds are still enabled, disable them in the dashboard to prevent duplicate deploys when clone works again.
