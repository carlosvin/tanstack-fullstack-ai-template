...
**CI/CD (Netlify-native):** GitHub Actions validates PRs and `main` (`.github/workflows/ci.yml`: lint, test, build; `.github/workflows/skills.yml`: skill drift, static skill evals, and [Waza](https://microsoft.github.io/waza/) readiness). Netlify Git integration handles all deploys — **deploy previews** on pull requests and **production** on merge to `main` (`netlify.toml` → `pnpm build`, publish `dist`). With `NETLIFY=true` (set automatically on Netlify), `@netlify/vite-plugin-tanstack-start` writes static assets to `dist/` and SSR to `.netlify/`; local non-Netlify builds still emit `.output/public`. No `NETLIFY_AUTH_TOKEN` secrets in GitHub. In Netlify: production branch `main`, deploy previews on, branch deploys off.

**Versioning (SemVer):** App version lives in `package.json` and is exposed via `shellSession.app.version`. Releases are automated with [Release Please](https://github.com/googleapis/release-please) (`.github/workflows/release-please.yml` + `release-please-config.json` + `.release-please-manifest.json`). Use [Conventional Commits](https://www.conventionalcommits.org/) on every commit (`feat:`, `fix:`, `BREAKING CHANGE` / `type!:` for major). PR commits are linted by `.github/workflows/commitlint.yml`. On push to `main`, Release Please opens/updates a **Release PR** that bumps `package.json`, updates `CHANGELOG.md`, and — when merged — tags `vX.Y.Z` and creates a GitHub Release. Do **not** hand-edit the version in feature PRs. Deploys stay on Netlify; the UI shows the version from the commit Netlify builds (typically the merged Release PR). No npm publish (`private: true`). If Release Please PRs skip CI (GITHUB_TOKEN limitation), configure a PAT/GitHub App token as `RELEASE_PLEASE_TOKEN` and pass it to the action.
...
`requireAuthMiddleware` chains `authMiddleware` so `context.accessTicket` is typed in mutation handlers.
...
pnpm skills:waza # Waza skill readiness + spec coverage + mock evals (needs `waza` CLI)
...
- **Deploys are Netlify-only.** PR previews and production deploys come from Netlify Git integration, not GitHub Actions. GHA `ci.yml` validates on PRs and `main`; `skills.yml` validates published skills (including Waza); `commitlint.yml` lints Conventional Commits on PRs; `release-please.yml` opens/updates Release PRs on push to `main`. Netlify publishes. Confirm deploy previews are enabled in the Netlify site settings.
- **SemVer releases:** Conventional Commits → Release Please Release PR → merge tags `vX.Y.Z`. Version in the running app comes from `package.json` at Netlify build time.
