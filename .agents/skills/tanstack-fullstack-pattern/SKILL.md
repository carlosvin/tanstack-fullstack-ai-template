...
17. AI renders rich markdown in the chat drawer: use `react-markdown` + `remark-gfm` to render assistant messages. Tables, code blocks, and lists are styled via a `.markdown` CSS Module class using Mantine CSS variables (not Mantine UI components). Internal links render as TanStack Router `Link` components (via a custom `MarkdownLink` renderer) with `preload="intent"` — clicking navigates the app without closing the drawer or losing chat history, because the drawer is mounted at the root layout level (rules 16 and 20). External links open in a new tab. The AI is instructed to use markdown links in replies (e.g. `[Pending tasks](/tasks?status=pending)`).
