...
import { parseTaskPriority, parseTaskStatus, TASK_PRIORITIES, TASK_STATUSES } from '../../constants/options'
			...
			<Stack gap="lg">
				...
				<Flex gap="sm" align="flex-end" direction={{ base: 'column', sm: 'row' }} wrap="wrap">
					...
					<Select
						...
						onChange={(val) => onUpdateSearch({ status: parseTaskStatus(val) })}
						w={{ base: '100%', sm: 150 }}
					/>
					<Select
						...
						onChange={(val) => onUpdateSearch({ priority: parseTaskPriority(val) })}
						w={{ base: '100%', sm: 150 }}
					/>
				</Flex>
